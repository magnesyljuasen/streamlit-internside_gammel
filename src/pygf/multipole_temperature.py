...
def multipole_temperature_app():
    ...
    filePath = 'src\pygf\data\ClaHel11_multipole_temperature.txt'
    ...
    (R, Rd) = gt.pipes.thermal_resistances(pos_pipes, rp_out, r_b, k_s, k_g,
                                           R_fp, J=3)
    st.write(50*'-')
    st.write('Thermal resistance:\t\t100*R11\t100*R12\t100*R22')
    st.write(f'Claesson and Hellstrom:\t{100*R_Claesson[0]:.3f}'
          f'\t{100*R_Claesson[1]:.3f}\t{100*R_Claesson[2]:.3f}')
    st.write(f'Present:\t\t\t\t{100*R[0,0]:.3f}\t{100*R[0,1]:.3f}'
          f'\t{100*R[1,1]:.3f}')
    st.write(50*'-')
