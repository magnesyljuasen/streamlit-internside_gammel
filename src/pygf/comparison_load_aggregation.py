...
def comparison_load_aggregation_app():
    # -------------------------------------------------------------------------
    ...
    for n, (LoadAgg, label) in enumerate(zip(LoadAggSchemes, loadAgg_labels)):
        st.write(f'Simulation using {label} ...')
    ...
    for label, maxError_n, t_calc_n in zip(loadAgg_labels, maxError, t_calc):
        st.write()
        st.write((f' {label} ').center(60, '-'))
        st.write(f'Maximum absolute error : {maxError_n:.3f} degC')
        st.write(f'Calculation time : {t_calc_n:.3f} sec')
