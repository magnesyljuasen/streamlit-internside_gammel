...
def custom_borehole_app():
    ...
    st.write(f'The geometry of the borehole is valid (realistic/possible): '
          f'{check_single!s}.')
    ...
    st.write(f'Single U-tube Borehole thermal resistance: '
          f'{R_b:.4f} m.K/W')
    ...
    R_b_series = DoubleUTube_series.effective_borehole_thermal_resistance(
        m_flow_borehole, fluid.cp)
    st.write(f'Double U-tube (series) Borehole thermal resistance: '
          f'{R_b_series:.4f} m.K/W')
    R_b_parallel = DoubleUTube_parallel.effective_borehole_thermal_resistance(
        m_flow_borehole, fluid.cp)
    st.write(f'Double U-tube (parallel) Borehole thermal resistance: '
    ...
    R_b = Coaxial.effective_borehole_thermal_resistance(
        m_flow_borehole, fluid.cp)
    st.write(f'Coaxial tube Borehole thermal resistance: {R_b:.4f} m.K/W')

    # Visualize the borehole geometry and save the figure
    fig_coaxial = Coaxial.visualize_pipes()
    #fig_coaxial.savefig('coaxial-borehole.png')
    st.pyplot(fig_coaxial)
