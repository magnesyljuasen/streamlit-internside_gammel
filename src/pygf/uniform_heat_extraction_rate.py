...
def uniform_heat_extraction_rate_app():
    # -------------------------------------------------------------------------
    ...
    filePath = 'src\pygf\data\CiBe14_uniform_heat_extraction_rate.txt'
    ...
    for i, field in enumerate([boreField1, boreField2, boreField3]):
        ...
        if i == 1:
            st.pyplot(gfunc.visualize_temperatures(iBoreholes=[18, 12, 14]))
            st.pyplot(gfunc.visualize_temperature_profiles(iBoreholes=[14]))
