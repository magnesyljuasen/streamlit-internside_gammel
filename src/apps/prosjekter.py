...
def insert_data(project_name, id, project_type, project_status, lat, long, name, date, percentage, checked_sendttilbud, checked_registrertoppdrag, checked_oppdragsbekreftelse, checked_sendtrapport, checked_faktura, workers):
    return db.put({
        "key" : project_name,
        "Oppdragsnummer" : id, 
        "Type" : project_type,
...
def new_project(name):
    with st.form("new_entry", clear_on_submit=True):
        ...
        with c2:
            ...
            project_status = st.multiselect("*Oppdragsstatus", ["Pågår", "Tilbud", "Fremtidig", "Fullført",])
        ...
        if st.form_submit_button("Oppdater"):
            ...
            insert_data(project_name, project_id, project_type[0], project_status[0], lat, long, name, date_today, percentage, checked_sendttilbud, checked_registrertoppdrag, checked_oppdragsbekreftelse, checked_sendtrapport, checked_faktura, workers)
        ...
        with st.form("change", clear_on_submit=False):
            if len(selected) > 0:
                ...
                with c2:
                    ...
                    project_status = st.selectbox("*Oppdragsstatus", ["Pågår", "Tilbud", "Fremtidig", "Fullført"], index=selected_index)
            ...
            if st.form_submit_button("Oppdater"):
                date_today = date.today().strftime("%d/%m/%Y")
                insert_data(project_name, project_id, project_type, project_status, lat, long, name, date_today, percentage, checked_sendttilbud, checked_registrertoppdrag, checked_oppdragsbekreftelse, checked_sendtrapport, checked_faktura, workers)
...
def map(df, color_pick):
    ...
    layers=[init], tooltip={
    'html': '<b>{key}</b> <br> {Type}, {Oppdragsnummer} <br> Ansvar: {Medarbeidere} <br> {Prosent}% fullført',
    'style': {'color': 'white'}}))
...
def prosjekter_app(name):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Pågår", "Tilbud", "Fremtidig", "Fullført"])
